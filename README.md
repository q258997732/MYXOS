...
MYXOS 是面向 MYTOS 设备的自监控与运维系统，采用 Spring Boot 2.7.x + JDK 1.8 多模块架构，支持设备发现、模板化指标采集、阈值告警、手动/自动操作任务与数据清理。
...
- 设备自动发现与手动托管
- 指标目录、指标模板与主机/安卓实例绑定的独立频率采集
- 受控只读 ADB 指标采集，包括 Android 基础信息、CPU、内存、任务数、最近应用和应用进程状态
- 应用进程状态监控：按设备、安卓实例和应用包名隔离 `FOREGROUND`、`ACTIVE`、`RUNNING`、`STOPPED` 快照与阈值
- 类型化阈值规则与告警管理，告警恢复后自动标记为 `RESOLVED` 并记录恢复时间
