...
3. **手动操作**：操作按钮网格，按用户最终确认范围包括：
   - 主机级：重启主机
   - 容器生命周期：运行、停止、重启、重置、重命名安卓容器
   - 安卓实例内部：设备截图（临时查看）、执行 Adb 命令、设置/获取剪贴板、设置系统语言、IP 智能定位
4. **最近告警**：该设备最近 20 条告警
...
- 除"设备截图"为即时查询、临时展示外，其余操作提交后调用 `POST /api/devices/{id}/ops` 进入异步任务队列，前端提示"任务已提交"。
- 任务结果通过"任务记录"Tab 异步刷新查看。
- 不保留 S5 代理、指纹更新、容器删除、备份/清理/排障等高风险或复杂操作。
...
| 操作类型 | REBOOT_HOST / RUN_ANDROID / STOP_ANDROID / REBOOT_ANDROID / RESET_ANDROID / RENAME_ANDROID / SET_CLIPBOARD / GET_CLIPBOARD / SET_LANGUAGE / REFRESH_LOCATION / SHELL_ADB 等（仅操作动作） |
...
左右分栏：
- 左侧：手动添加表单
  - IP / 端口 / 模式 / 分组 / 名称（可选，留空自动从 `healthcheck`/`systeminfo` 读取主机信息回填）
  - 保存按钮
- 右侧：网段发现
