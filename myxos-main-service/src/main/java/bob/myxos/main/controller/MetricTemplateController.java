...
public class MetricTemplateController {
    ...
    @GetMapping @PreAuthorize("hasRole('ADMIN')") public Result<List<MetricTemplate>> list() { return Result.ok(metricTemplateService.list()); }
    @GetMapping("/{id}") @PreAuthorize("hasRole('ADMIN')") public Result<Map<String,Object>> detail(@PathVariable Long id) { Map<String,Object> d=new HashMap<String,Object>(); d.put("template", metricTemplateService.detail(id)); d.put("items", metricTemplateService.listItems(id)); return Result.ok(d); }
